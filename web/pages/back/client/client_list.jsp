...
<script type="text/javascript" src="/js/pages/back/client/client_list.js"></script>
</head>
<body>
<body class="sarchbody">
	...
	<div class="formbody">
					...
					<c:if test="${allClients!= null}">
						<c:forEach items="${allClients}" var="client">
							<tr>
								...
								<td><a href="<%=basePath%>${allActions['3'].url}?client.cid=${client.cid}" class="tablelink">修改</a></td>
                                </c:if>

                                <c:if test="${allActions['5']!=null}">
								<td><a href="<%=basePath%>${allActions['5'].url}?cid=${client.cid}&name=${client.name}" class="tablelink">添加任务</a></td>
                                </c:if>

                                <c:if test="${allActions['30']!=null}">
                                    <td><a href="<%=basePath%>${allActions['30'].url}?cid=${client.cid}&name=${client.name}" class="tablelink">任务列表</a></td>
                                </c:if>
							</tr>
						</c:forEach>
					</c:if>
		...
		<jsp:include page="/pages/plugins/split_page_bar_plugin_mvc.jsp"/>
