...
<script type="text/javascript" src="/js/pages/back/task/task_list.js"></script>
					...
					<c:forEach items="${allTasks}" var="task">
						<tr>
							...
							<td>
                                ...
                                <a href="<%=basePath%>${allActions['7'].url}?tid=${task.tid}">修改</a>
                                </c:if>
                            </td>
                            <td>
                                <c:if test="${task.status==1}">
                                <c:if test="${allActions['8'] != null}">
                                    <a href="<%=basePath%>${allActions['8'].url}?tid=${task.tid}">完成</a>
                                </c:if>
                                </c:if>
                            </td>
                            <td>
                                ...
                                <c:if test="${allActions['9'] != null}">
                                    <a href="<%=basePath%>${allActions['9'].url}?tid=${task.tid}">关闭</a>
                                </c:if>
                                </c:if>
                            </td>
						</tr>
        ...
        <c:if test="${allActions['10']!=null}">
            <div class="buttonform1">
                <li class="delbtn"><label>&nbsp;</label>
                    <input id="delClient" type="button" class="deltn" value="删除任务" />
            </div>
        </c:if>
